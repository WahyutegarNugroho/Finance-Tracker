...
export default function Budget() {
  ...
  const { data: budgetsData, isLoading: budgetsLoading, isError: budgetsError } = useQuery<ApiResponse<BudgetWithSpent[]>>({
    ...
    <div className="animate-in fade-in slide-in-from-bottom-4 duration-700">
      ...
      {summary && (
        <section className="glass-card bg-surface-container/70 rounded-xl p-6 md:p-8 mb-8 border border-outline-variant/30 relative overflow-hidden">
          ...
          <div className="relative w-full h-4 bg-surface-variant/80 rounded-full overflow-hidden border border-white/20 shadow-inner z-10">
            <div className={`absolute top-0 left-0 h-full rounded-full transition-all duration-1000 ease-out ${summary.overallPercentage >= 95 ? 'bg-error' : summary.overallPercentage >= 75 ? 'bg-tertiary' : 'bg-primary'}`}
              style={{ width: `${Math.min(100, summary.overallPercentage)}%` }}></div>
          </div>
        </section>
      )}
      ...
      {(!Array.isArray(budgets) || budgets.length === 0) ? (
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-4 gap-4 md:gap-6">
          {budgets.map((b) => {
            let statusColor = "text-primary", bgColor = "bg-primary", bgContainer = "bg-primary/10";
            if (b.status === "critical") { statusColor = "text-error"; bgColor = "bg-error"; bgContainer = "bg-error/10"; }
            else if (b.status === "warning") { statusColor = "text-tertiary"; bgColor = "bg-tertiary"; bgContainer = "bg-tertiary-container/20"; }
            else if (b.status === "good") { statusColor = "text-secondary"; bgColor = "bg-secondary"; bgContainer = "bg-secondary/10"; }
            ...
            return (
              <div key={b.id} className={`glass-card bg-surface-container-low/80 rounded-xl p-5 border ${b.status === 'critical' ? 'border-error/20 hover:border-error/40' : 'border-outline-variant/20 hover:shadow-md'} transition-shadow duration-300 flex flex-col justify-between h-48 relative overflow-hidden group`}>
                {b.status === 'critical' && <div className="absolute top-0 right-0 w-16 h-16 bg-error/5 rounded-bl-full pointer-events-none"></div>}
                <div className="absolute top-2 right-2 opacity-0 group-hover:opacity-100 transition-opacity z-20 flex gap-1">
                  <button onClick={() => { setBudgetToEdit(b); setIsModalOpen(true); }}
                    className="p-1.5 bg-surface rounded-md border border-outline-variant/20 hover:bg-surface-variant text-on-surface transition-colors" title="Edit Budget">
                    <span className="material-symbols-outlined text-[16px]">edit</span>
                  </button>
                  <button onClick={() => handleDeleteClick(b.id)}
                    className="p-1.5 bg-surface rounded-md border border-outline-variant/20 hover:bg-error-container hover:text-error text-on-surface transition-colors" title="Delete Budget">
      ...
      <ConfirmDialog 
        ...
        onCancel={() => { setIsConfirmOpen(false); setBudgetToDelete(null); }}
      />
    </div>
  );
}
